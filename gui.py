    ...
    try:
        ...
        if threat_clicked:
            ...
            if "error" in threat_info:
                ...
                threat_json = json.dumps(threat_info, indent=2).replace('{', '<span style="color:#9c27b0;">{</span><br>').replace('}', '<br><span style="color:#9c27b0;">}</span>').replace(',', ',<br>').replace('"', '<span style="color:#1976d2">"</span>').replace(':', '<span style="color:#616161">:</span>')
                st.markdown(f"""
